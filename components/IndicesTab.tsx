...
const IndicesTab: React.FC<Props> = ({ data, onRefresh, loading }) => {
  ...
  const isStale = (new Date().getTime() - new Date(data.updated_at).getTime()) > (10 * 24 * 60 * 60 * 1000);
  ...
  return (
    <Container>
      ...
      <Section title="Andamento 12 Mesi">
        <div className="h-64 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Tooltip />
              <Legend />
              <Line yAxisId="left" type="monotone" dataKey="PUN" stroke="#2dd4bf" strokeWidth={2} dot={false} name="PUN (Luce)" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <div className="bg-white p-4 rounded shadow-sm">
          <div className="flex justify-between items-center mb-4">
             <h4 className="font-bold text-brand-primary">PUN (Luce)</h4>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500 border-b">
                ...
                <th className="pb-2 text-right">Valore (€/kWh)</th>
              </tr>
            </thead>
            <tbody>
